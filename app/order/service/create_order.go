...
import (
	"context"
	"github.com/PokemanMaster/GoChat/server/app/order/dao"
	model2 "github.com/PokemanMaster/GoChat/server/app/order/model"
...
func (service *CreateOrderService) Create(ctx context.Context) *resp.Response {
	...
	//生成随机订单号
	number := model2.RandomNum(service.ProductID, service.UserID)
	order.Code = number
	...
	orderDetail := model2.OrderDetail{
		OrderID:     order.ID,
